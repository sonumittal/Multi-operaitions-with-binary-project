...
def G():
    w = t.Toplevel()
    w.title("Excess 3 Addition and Subtraction")
    w.geometry("444x234")
    ...
    btn = t.Button(w,text="Calculate",command=lambda:excess(en1.get(),en2.get(),add_out_dec,add_out_bin,sub_out_dec,sub_out_bin,l))
...
def excess(n1,n2,add_dec,add_bin,sub_dec,sub_bin,error):
    a=n1
    if not a.isdigit():
